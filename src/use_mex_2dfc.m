...
% fcont_gram_blend_S, inpolygon_mesh, barylag) checks this before calling
% into its mex gateway, falling back to the original pure-MATLAB
% implementation otherwise. This keeps the repository fully usable (just
% slower) on a machine where `build_mex` hasn't been run or mex compilation
% isn't available, and keeps every public function's behavior identical
% either way -- only the implementation backing it changes.
%
