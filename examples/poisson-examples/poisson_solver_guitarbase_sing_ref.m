...
n_y     = 2187;
int_eps = 1e-14;
num_path = 'poisson_solver_guitarbase_sing_h2.mat';
save_path = 'poisson_solver_guitarbase_sing_h2_ref.mat';

h_ref = h/2;

f          = @(x, y) (3*pi)^2 * sin(3*pi*x - 1) .* sin(3*pi*y - 1);
u_boundary = @(x, y) (x/2).^2+(y/2).^2;
...
h_tan = 2 * h_ref;   h_norm = h_tan;   n_curve = 0;
...
tic;
rng('default')
[u_exact_mat, ~, R_eval] = poisson_solver_coarse(curve_seq, f, u_boundary, h_ref, 1, p, int_eps, 1e-13, 1e-13, d, C, n_r, A, Q, M, R, n_x, n_y, true);
toc;

save(save_path, 'R_eval', 'u_exact_mat');